...
def extract_transform_f():
    # PoC: Extract data from CSV file
    # ======================================
    headers = ['timestamp','store','customer_name','basket_items','total_price','cash_or_card','card_number']
    df = pd.read_csv("longridge-test.csv", parse_dates=['timestamp'], infer_datetime_format=True, names = headers)
    ...
    df_1nf["product_name"] = [x.rsplit(" -", 1)[0] for x in df_1nf["basket_items"]]
    ...
    df_1nf.index = df_1nf.index 
    df_2nf = df_1nf
    
    df_2nf.index += max_order_id()
    df_2nf["order_id"] = df_2nf.index
   

    # 2.2 Create product_id & apply to df_2nf table
    ...
    df_2nf['product_id'] = df_2nf['product_name'].apply(product_names.get) + 1 # assigning each product in df_1nf with their corresponding id
  
    # 2.3 Create store_id by strippin first 5 letters from store name and assigning to store_id
    df_2nf['store_id'] = [x[0:5] for x in df_2nf['store']]
    # print(df_2nf)
     # 2.4 Create products table (transforming and extracting from df_2nf)
    stores_df = df_2nf.drop_duplicates(subset = ["store"])
    stores_df = stores_df[["store_id","store"]]
 
    
   
    # 2.5 Create orders table (transforming and extracting from df_2nf)
    # NOTE: should not overwrite exisiting df_2nf as may lose some products
    orders_df = df_2nf.drop_duplicates(subset = ["order_id"]) # removing duplicate order ids in df_2nf and returned as a copy (inplace=False) called orders_df
    orders_df = orders_df[['order_id','order_date','order_time','store_id','total_price','cash_or_card']]
   

    # 2.6 Create order_item table
    order_item_df = df_2nf.groupby(['order_id','product_id','store_id','product_name'])['order_time'].count().reset_index(name='quantity')
   
    
    # 2.7 Create products table (transforming and extracting from df_2nf)
    ...
    return stores_df, orders_df, products_df, order_item_df
